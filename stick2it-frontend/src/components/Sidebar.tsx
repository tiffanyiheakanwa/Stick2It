import { LayoutDashboard, CheckSquare, Calendar, TrendingUp, Award, Sparkles, LogOut, X } from "lucide-react";
...
export function Sidebar({ activeSection, onSectionChange, isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside className={`w-64 bg-gradient-to-b from-indigo-600 to-indigo-700 h-screen fixed left-0 top-0 flex flex-col text-white z-50 transition-transform duration-300 ${
        isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
      }`}>
        {/* Close button for mobile */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-2 hover:bg-indigo-500 rounded-lg lg:hidden"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="p-6">
          <div className="flex items-center gap-2 mb-8">
            ...
            <span className="text-white">RemindAI</span>
          </div>
